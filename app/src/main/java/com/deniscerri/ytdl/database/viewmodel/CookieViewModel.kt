...
class CookieViewModel(private val application: Application) : AndroidViewModel(application) {
    ...
    // Imports can finish together; serialize regeneration so no update is lost.
    private val cookieFileMutex = Mutex()
    ...
    /** Login recovery awaits this so the worker cannot resume before the jar is on disk. */
    suspend fun updateCookiesFileNow() {
        persistCookiesFile()
    }

    private suspend fun persistCookiesFile() = cookieFileMutex.withLock {
        val content = CookieFileUtil.merge(repository.getAllEnabled())
        var changed = false
        FileUtil.getCookieFile(application, true) { path ->
            changed = CookieFileUtil.writeAtomicallyIfChanged(File(path), content)
        }

        if (changed) {
            // Cached metadata may represent a different authenticated session.
            File(FileUtil.getInfoJsonPath(application)).deleteRecursively()
        }
    }
